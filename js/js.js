...
$(document).ready(function(){
	...
	var verifyPlaylist = false;
	var verifyConfig = false;

	$.getJSON("json/config.json", function(data){
		verifyPlaylist = true;
		audioPlayer.config = data;
	}).done(function(){
		$.getJSON("json/playlist.json", function(data){
			verifyConfig = true;
			audioPlayer.playlist = data;
		}).done(function(){
			// Call functions
	...
	setTimeout(function(){
		if(!verifyConfig){
			console.log("Error: failed true response from playlist within "+responseTime+"ms");
		}
		if(!verifyPlaylist){
			console.log("Error: failed true response from config within "+responseTime+"ms");
		}
	}, responseTime);
});
